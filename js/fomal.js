...
function tonav() {
  ...
  document.getElementById("page-name").innerText = document.title.split(" | Xvzhang🥝")[0];
}
...
function showWelcome() {
  ...
  let dist = getDistance(103.718953, 36.104493, ipLoacation.result.location.lng, ipLoacation.result.location.lat); //这里换成自己的经纬度
...
function createtime1() {
  var grt = new Date("18/03/205 00:00:00"); //此处修改你的建站时间或者网站上线时间
...
function share_() {
  ...
  try {
    ...
    navigator.clipboard.writeText('序章🥝的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！🍭🍭🍭');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
...
function createtime() {
  ...
  var mnum = String(Math.floor((diff % 3600) / 60)).padStart(2, "0"); // 分钟
  var snum = String(diff % 60).padStart(2, "0"); // 秒数

  // 判断时间段
  var workImg =
    hnum < 18 && hnum >= 9
      ? "X小屋-科研摸鱼中.svg"
      : "X小屋-科研摸鱼中.svg"; // 图片地址相同，保留占位
  ...
  // 生成 HTML 内容
  var currentTimeHtml = `
    <img class='boardsign' src='https://raw.githubusercontent.com/xvzhnang/xvzhnang.github.io/092ec2fffde2e546737f69b23dc9f428a6abbc66/svg/${workImg}' title='${workTitle}'>
    <br> 
